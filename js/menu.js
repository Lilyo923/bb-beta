...
].filter(e => !(BETA && e.cle === 'avances'));   // pas d'outils de dev en beta
...
function menuValider() {
    ...
    if (e.cle === 'retour') { menuRetour(); }
      ...
      if (partie.existe) {
        demanderConfirmation(
          ...
          // Ce qui NE s'efface pas merite d'etre dit aussi : sinon le joueur
          // croit perdre son skin exclusif et renonce a recommencer.
          + (recompenseObtenue('beta-testeur')
              ? ' (Le skin Bêta-testeur, lui, reste à toi.)' : ''),
          () => {
            effacerPartie();
  ...
  if (e.cle === 'nouvelle') {
    ...
    const nouvellePartie = () => {
      if (partie.existe) {
        demanderConfirmation(
          ...
          // En beta, dire ce qui NE s'efface PAS : le skin exclusif.
          + (BETA && recompenseObtenue('beta-testeur')
              ? ' Le skin Bêta-testeur, lui, reste à toi.' : ''),
    ...
    // En beta, l'avertissement passe d'abord ; le reste est inchange.
    if (BETA) ouvrirAvisBeta(nouvellePartie);
    else nouvellePartie();
...
function rentrerALaBase() {
  // La fin de la beta s'intercale ici, une seule fois : entre l'ecran de fin
  // du troisieme niveau et le retour a la base.
  if (finBeta.aAfficher) { ouvrirFinBeta(); return; }
...
function dessinerMenu() {
  ...
  dessinerSplashBeta();          // « Bêta ! » en diagonale — rien en version finale
  ...
  ctx.fillText('v' + versionLisible(), LARGEUR - 8, HAUTEUR - 8);
  ctx.textAlign = 'left';
...
/* -----------------------------------------------------------------------------
   LA MISE EN PAGE DES OPTIONS

   Le cadre faisait 46 a 314 et les lignes commençaient a 96, hautes de 28 (34
   pour la difficulte). A six entrees ça tenait. A dix — reglages, difficulte,
   retablir, effacer, controles, politique, contact, developpement, retour — il
   faut 286 pixels pour 218 disponibles : les deux dernieres lignes tombaient
   sous le cadre, et « Retour » sortait meme de l'ecran.

   Deux corrections, pas une :

   1. Le cadre prend les memes bornes que les panneaux de la base (26 a 334) au
      lieu d'etre plus petit qu'eux.
   2. La hauteur d'une ligne se DEDUIT de la place et du nombre d'entrees. Une
      onzieme entree resserrera la liste ; elle ne la fera pas deborder. C'est
      la difference entre un reglage qui marche aujourd'hui et un reglage qui
      marche encore au prochain ajout.
-------------------------------------------------------------------------- */
const OPT_Y0 = 66;            // premiere ligne
const OPT_Y1 = 298;           // fin de la zone des lignes
const OPT_MESSAGE_Y = 312;
const OPT_AIDE_Y = 327;

/* Les hauteurs « naturelles », celles qu'on voudrait s'il y avait la place. */
function hauteursOptions() {
  // La ligne « difficulte » porte une note sous elle : il lui faut deux
  // interlignes, pas un.
...
function dessinerOptions() {
  dessinerDemo();
  cadre(48, 26, LARGEUR - 96, HAUTEUR - 52);
  texteCentre('OPTIONS', 48, 'bold 17px system-ui, sans-serif', '#e8b62c');

  const JAUGE_L = 110;
  const JAUGE_FIN = LARGEUR - 104;      // le pourcentage tient a droite du rail
  const JAUGE_X = JAUGE_FIN - JAUGE_L;

  const hauteurs = hauteursOptions();
  let y = OPT_Y0;
  MENU_OPTIONS.forEach((e, i) => {
    const hauteurLigne = hauteurs[i];
    ...
    /* La plaque de selection ne couvre que la ligne du libelle : sur la ligne
       « Difficulte », la note qui la suit doit rester LISIBLE, donc hors de
       l'aplat ambre. */
    const hPlaque = Math.min(hauteurLigne - 3, 22);
    if (actif) {
      ctx.fillStyle = 'rgba(232,182,44,.13)';
      ctx.fillRect(64, y - 12, LARGEUR - 128, hPlaque);
    }
    ctx.font = (actif ? 'bold ' : '') + '12px system-ui, sans-serif';
    ctx.fillStyle = actif ? '#ffe9a8' : 'rgba(255,255,255,.72)';
    ctx.fillText(e.nom, 76, y + 4);
    ...
    zone(64, y - 12, LARGEUR - 128, hauteurLigne - 3, 'option-ligne', i);
    ...
    if (e.genre === 'jauge') {
      ...
      /* Le pourcentage etait ecrit SOUS le rail, a y + 18. Avec des lignes
         resserrees, il tombait dans la ligne suivante. Il est maintenant a
         DROITE du rail, sur la meme ligne de base que le nom du reglage. */
      ctx.font = '10px system-ui, sans-serif';
      ctx.textAlign = 'right';
      ctx.fillStyle = 'rgba(255,255,255,.5)';
      ctx.fillText(Math.round(v * 100) + ' %', LARGEUR - 64, y + 4);
      ctx.textAlign = 'left';
      // Zone genereuse en hauteur : on vise une barre de 5 px avec une souris.
      zone(JAUGE_X - 8, y - 12, JAUGE_L + 16, hauteurLigne - 3, 'jauge', i);
      ...
      const centre = JAUGE_X + JAUGE_L / 2;
      ctx.fillStyle = '#e8b62c';
      ...
      [[-1, centre - 74, '‹'], [1, centre + 74, '›']].forEach(([pas, fx, glyphe]) => {
        ...
        zone(fx - 13, y - 12, 26, Math.min(24, hauteurLigne - 3), 'choix', pas);
      });

      ctx.textAlign = 'left';
      ctx.font = '9px system-ui, sans-serif';
      ctx.fillStyle = 'rgba(255,255,255,.36)';
      // La note colle au BAS de sa ligne : ainsi elle ne mord jamais sur la
      // suivante, quelle que soit la hauteur calculee.
      ctx.fillText(d.note, 76, y + hauteurLigne - 13);
    }
  ...
  if (messageMenuT > 0) {
    texteCentre(messageMenu, OPT_MESSAGE_Y, '11px system-ui, sans-serif', '#7ee08a',
                Math.min(1, messageMenuT));
  }
  texteCentre('← → pour régler · clic direct sur les jauges · Échap pour revenir',
              OPT_AIDE_Y, '10px system-ui, sans-serif', 'rgba(255,255,255,.34)');
...
function activerZone(z) {
  ...
  switch (z.action) {
    ...
    /* Le bandeau juridique. Lire le texte ne le fait PAS disparaitre : sinon
       celui qui clique par curiosite, lit, et revient par Echap n'a plus aucun
       moyen d'y retourner depuis l'accueil. Seul « Jouer » vaut acceptation —
       c'est ce que la phrase annonce, et c'est `lancerDemarrage` qui le
       marque. */
    case 'mentions-lien':    ouvrirMentions('accueil'); break;
    case 'mentions-retour':  fermerMentions(); break;
    case 'mentions-haut':    pageMentions(-1); break;
    case 'mentions-bas':     pageMentions(1); break;
    case 'controles-retour': fermerControles(); break;
    case 'avisbeta-ok':      validerAvisBeta(); break;
    case 'avisbeta-retour':  annulerAvisBeta(); break;
    case 'finbeta-ok':       quitterFinBeta(); break;
    case 'ecrire-studio':    ecrireAuStudio(); break;
