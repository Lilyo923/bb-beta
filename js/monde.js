...
const PLANCHES_UNIFORMES = {
  ...
  'beta-testeur':     'assets/brad/brad-beta-testeur.png',
};
...
function appliquerUniforme(cle) {
  /* On ne porte que ce qui est ouvert. Une partie venue d'une version de test
     pouvait porter le costume d'or ; en beta, il n'existe pas encore. La partie
     garde son choix (il reviendra a la sortie) — seul l'affichage retombe sur
     le classique. */
  if (typeof UNIFORMES !== 'undefined' && typeof uniformeDebloque === 'function') {
    const u = UNIFORMES.find(x => x.cle === cle);
    if (!u || !uniformeDebloque(u)) cle = 'classique';
  }
